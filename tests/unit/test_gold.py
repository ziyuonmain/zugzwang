...
def test_gold_grain_preservation(spark: SparkSession):
    """Tests that Gold multi-domain enrichment preserves exact 1:1 stop grain without row multiplication."""
    stops_schema = StructType(
        [
            StructField('id', StringType(), False),
            StructField('eva', StringType(), False),
    ...
    stops_data = [
        (
            ...
            False,
            False,
            datetime(2026, 6, 1, 14, i, 0),
            datetime(2026, 6, 1, 12, i, 0),
            datetime(2026, 6, 1, 12, 0, 0),
        )
        for i in range(10)
    ]
    ...
    stations_data = [('08000105', 'Frankfurt (Main) Hbf', 1, 1, 'Hessen')]
    df_stations = spark.createDataFrame(stations_data, schema=stations_schema)
    ...
    mapping_data = [('08000105', '01420', 4.54, '01420', 4.54)]
    df_mapping = spark.createDataFrame(mapping_data, schema=mapping_schema)
    ...
    tu_data = [('01420', datetime(2026, 6, 1, 12, 0, 0), 22.4, 45.0, 1)]
    df_tu = spark.createDataFrame(tu_data, schema=tu_schema)
    ...
    ff_data = [('01420', datetime(2026, 6, 1, 12, 0, 0), 3.2, 230, 1)]
    df_ff = spark.createDataFrame(ff_data, schema=ff_schema)
    ...
    assert df_gold.count() == df_stops.count() == 10
